  ...
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const codepipeline = new pipelines.CodePipeline(this, 'CodePipeline', {
      synth: new ShellStep('ShellStep', {
        ...
        input: pipelines.CodePipelineSource.connection(`${constants.GITHUB_REPO_OWNER}/${constants.GITHUB_REPO_NAME}`, constants.GITHUB_REPO_BRANCH, { connectionArn: constants.GITHUB_CONNECTION_ARN }),
        commands: ['npm install', 'npm run build', 'npx cdk synth'],
      }),
    });

    const authentication = new Authentication(this, `${constants.APP_NAME}-Prod`, {
